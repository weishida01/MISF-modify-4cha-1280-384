...
class InpaintingModel(BaseModel):
    ...
    def process(self, images, masks):
        ...
        gen_content_loss = self.perceptual_loss(outputs[:,:3,:,:], images[:,:3,:,:])
        ...
        gen_style_loss = self.style_loss(outputs[:,:3,:,:] * masks, images[:,:3,:,:] * masks)
    ...
    def forward(self, images, masks):
        ...
        images_masked = images * (1 - masks)    # mask值为0 1 mask区域为 1
        inputs = torch.cat((images_masked, masks), dim=1)   # n 5 256 256
